...
#
#
#  En environnement VSC, utiliser UNIQUEMENT la version secure
#	voir plus bas $secure = 1
#
#  Version initiale adaptée du script récupéré sur Nagios Exchange
#       Written by Michael Streb <michael.streb@netways.de>
#
# 18/07/11 (JCU): Version initiale modifiée pour VSC
#
...
sub check_sp {
	...
	if ($secure eq 1 ) {
		open (NAVICLIOUT ,"$NAVISECCLI -secfilepath $NAVISECDIR -h $opt_host getcrus |");
	}
	my $sp_line = 0;
	my $error_count = 0;
	while (<NAVICLIOUT>) {
		...
		if ($sp_line == 1) {
			...
			if ($_ =~ m/Enclosure\s(\d+|\w+)\s(Power)\s($opt_sp\d?)\s\w+:\s+(\w+)/) {
				my $check = $2;
				my $device = $3;
				if ($4 =~ m/Present|Valid/) {
					$output .= "$check $device ok, ";
				} else {
					$output .= "$check $device failed, ";
					$error_count++;
				}	
			}
			if ($_ =~ m/Enclosure\s(\d+|\w+)\s(SPS)\s$opt_sp\d?\s\w+:\s+(\w+)/) {
                                my $check = $2;
...
sub check_disk {
	...
	if ($secure eq 1 ) {
		open (NAVICLIOUT ,"$NAVISECCLI -secfilepath $NAVISECDIR -h $opt_host getdisk -state |");
	}
	while (<NAVICLIOUT>) {
...
sub check_cache {
	...
	if ($secure eq 1 ) {
		open (NAVICLIOUT ,"$NAVISECCLI -secfilepath $NAVISECDIR -h $opt_host getcache |");
	}
	while (<NAVICLIOUT>) {
		# check for cache
...
sub check_faults {
	$state = 'UNKNOWN';
	if ($secure eq 0 ) {
		print "The check of the faults only works with Naviseccli. Please use secure version !\n";
		exit $states{$state};
	}
	if ($secure eq 1 ) {
		open (NAVICLIOUT ,"$NAVISECCLI -secfilepath $NAVISECDIR -h $opt_host Faults -list |");
	}
	while (<NAVICLIOUT>) {
...
sub check_portstate {
	...
	if ($secure eq 1 ) {
		open (NAVICLIOUT ,"$NAVISECCLI -secfilepath $NAVISECDIR -h $opt_host getall -hba |");
	}
	...
	while (<NAVICLIOUT>) {
		...
		if ($opt_port >= 0) {
			if ($sp_line == 1 && $_ =~ m/^SP\sPort\sID:\s+($opt_port)$/) {
				$port_id = $1;
				$portstate_line = 1;
				$error_count = 0;
			} 
		} else {
			if ($sp_line == 1 && $_ =~ m/^SP\sPort\sID:\s+(\d+)$/) {
				$port_id = $1;
				$portstate_line = 1;
				$error_count = 0;
			} 
		...
		if ($sp_section == 1 && $sp_line == 1 && $portstate_line == 1) {
			...
			if ($_ =~ m/^Link\sStatus:\s+(.*)$/) {
				...
				my $status = $1; 
				if ($status =~ m/Up/) {
					$output .= " - Port: $port_id, Link: $status, ";
				} else {
					$output .= " - Port: $port_id, Link: $status, ";
					$error_count++;
				}	
	...
	if ($error_count == 0) {
		...
		$state='UNKNOWN';
		$output = 'UNKNOWN: specified port ($opt_port) not found ';
	} else {
		$state='CRITICAL';
	}
