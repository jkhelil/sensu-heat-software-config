...
class Mailer < Sensu::Handler
  ...
  def build_mail_to_list
    ...
    if settings[json_config].key?('contacts')
      @event['check']['contacts'].each do |sub|
        if settings[json_config]['contacts'].key?(sub)
          mail_to << ", #{settings[json_config]['contacts'][sub]['mail_to']}"
        end
      end
    end
    mail_to
